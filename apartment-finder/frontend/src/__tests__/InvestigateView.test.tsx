...
import type { ListingPreview, SearchResult } from '../types';
...
const mockSearchResults: { results: SearchResult[] } = {
  results: [
    { title: 'Result 1', address: 'Addr 1', city: '', price: 9000, size: 35, rooms: 1, photo: null, sourceUrl: 'https://qasa.se/homes/r1' },
  ],
};
...
describe('InvestigateView — URL tab', () => {
  ...
  it('shows scraped preview after submitting a URL', async () => {
    (fetch as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ ok: true, json: async () => mockListing });
    renderView();
  ...
  it('shows error message when scrape fails', async () => {
    (fetch as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ ok: false, json: async () => ({ error: 'Not found' }) });
    renderView();
...
describe('InvestigateView — Search tab', () => {
  ...
  it('shows search results', async () => {
    (fetch as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ ok: true, json: async () => mockSearchResults });
    renderView();
    await userEvent.click(screen.getByRole('tab', { name: /search/i }));
