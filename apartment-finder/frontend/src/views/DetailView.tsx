...
import type { Apartment, Priority, Status } from '../types';
...
export default function DetailView() {
  ...
  async function update(patch: Partial<Apartment>) {
    if (!apt) return;
  ...
  const saveNotes = useDebounce(async (text: string) => {
    setSaving(true);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="space-y-2">
        ...
        <PriorityPicker value={apt.priority} onChange={(p: Priority) => update({ priority: p })} />
      </div>

      <div className="space-y-2">
        <h2 className="text-sm font-semibold text-gray-500 uppercase">Status</h2>
        <StatusStepper value={apt.status} onChange={(s: Status) => update({ status: s })} />
      </div>
