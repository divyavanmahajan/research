...
function ResultCard({ listing, onAdd, alreadyAdded }) {
  return (
    <div className="bg-white rounded-xl shadow p-4 flex gap-4">
      <div className="w-24 h-20 flex-shrink-0 bg-gray-100 rounded-lg overflow-hidden">
        {listing.photo || listing.photos?.[0] ? (
          <img src={listing.photo ?? listing.photos[0]} alt={listing.address} className="w-full h-full object-cover" />
        ) : (
      ...
      <div className="flex-1 min-w-0 space-y-1">
        ...
        <p className="text-xs text-gray-500">{listing.address}{listing.city ? `, ${listing.city}` : ''}</p>
...
function SearchTab() {
  const [params, setParams] = useState({ city: '', minPrice: '', maxPrice: '', minSize: '', maxSize: '', rooms: '' });
  const [loading, setLoading] = useState(false);
  ...
  async function handleAdd(listing) {
    ...
    const full = res.ok ? await res.json() : listing;
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {[
          { key: 'city', placeholder: 'City (e.g. Stockholm)', type: 'text' },
          { key: 'minPrice', placeholder: 'Min price (kr)', type: 'number' },
          { key: 'maxPrice', placeholder: 'Max price (kr)', type: 'number' },
          { key: 'minSize', placeholder: 'Min size (m²)', type: 'number' },
          { key: 'maxSize', placeholder: 'Max size (m²)', type: 'number' },
          { key: 'rooms', placeholder: 'Rooms', type: 'number' },
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex border-b border-gray-200">
        {[{ value: 'url', label: 'Paste URL' }, { value: 'search', label: 'Search' }].map(t => (
          <button
            key={t.value}
            role="tab"
