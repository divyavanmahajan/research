...
import type { Apartment, Priority } from '../types';
...
type GeoApartment = Apartment & { lat: number; lng: number };

export default function MapView() {
  const [apartments, setApartments] = useState<GeoApartment[]>([]);

  useEffect(() => {
    getAll().then(all => setApartments(all.filter((a): a is GeoApartment => a.lat != null && a.lng != null));
  }, []);

  const positions: [number, number][] = apartments.map(a => [a.lat, a.lng]);
  const center: [number, number] = positions.length > 0 ? positions[0] : [59.33, 18.06];
